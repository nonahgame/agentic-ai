...
# New function to fetch recent signals from database
async def get_recent_signals(limit: int = 10) -> List[Dict[str, Any]]:
    """Fetch the most recent trading signals and messages from the database."""
    try:
        with db_lock:
            c = conn.cursor()
            c.execute('''
                SELECT time, action, symbol, price, rsi, ema1, ema2, message, timeframe
                FROM trades
...
async def dashboard():
    """Display a dashboard with strategy, portfolio, and recent signals."""
    strategy = await get_current_strategy()
    portfolio = await get_portfolio_balance()
    signals = await get_recent_signals(limit=10)
    data = await fetch_crypto_data(SYMBOL, TIMEFRAME, limit=1)
    
    # Generate HTML table for signals
    signals_table = "<table border='1' style='width:100%; border-collapse: collapse;'>"
    signals_table += "<tr><th>Time</th><th>Signal</th><th>Symbol</th><th>Price</th><th>RSI</th><th>EMA1</th><th>EMA2</th><th>Message</th><th>Timeframe</th></tr>"
    for signal in signals:
        signals_table += f"<tr><td>{signal['time']}</td><td>{signal['signal']}</td><td>{signal['symbol']}</td><td>{signal['price']:.2f}</td><td>{signal['rsi']:.2f}</td><td>{signal['ema1']:.2f}</td><td>{signal['ema2']:.2f}</td><td>{signal['message']}</td><td>{signal['timeframe']}</td></tr>"
    signals_table += "</table>"

    # Current market data
    market_data = data.get("indicators", {})
    ...
    <html>
        ...
        <body>
            ...
            <h2>Portfolio</h2>
            <pre>{portfolio}</pre>
            <h2>Current Market Data ({SYMBOL}, {TIMEFRAME})</h2>
            <pre>Close Price: {market_data.get('close_price', 'N/A'):.2f}<br>RSI: {market_data.get('rsi', 'N/A'):.2f}<br>EMA1: {market_data.get('ema1', 'N/A'):.2f}<br>EMA2: {market_data.get('ema2', 'N/A'):.2f}</pre>
...
async def get_signals(limit: int = 10):
    """Fetch recent trading signals and messages."""
    logger.info(f"Fetching {limit} recent signals")
    ...
    return {"signals": signals, "message": f"Retrieved {len(signals)} signals"}
